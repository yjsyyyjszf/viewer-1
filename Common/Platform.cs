...
namespace Macro.Common
{
    ...
    public static class Platform
    {
        ...
        private static string _configFolder = "Setting";
        private static string _failureFilesFolder = "failureFiles";
        ...
        private static volatile string _configDirectory = null;
        private static volatile string _failureDirectory = null;
            ...
            get
            {
                if (_configDirectory == null)
                {
                    lock (_syncRoot)
                    {
                        if (_configDirectory == null)
                        {
                            string configDirectory = Path.Combine(Platform.InstallDirectory, _configFolder);
        ...
        public static string FailureDirectory
        {
            get
            {
                if (_failureDirectory == null)
                {
                    lock (_syncRoot)
                    {
                        if (_failureDirectory == null)
                        {
                            string failureDirectory = Path.Combine(Platform.InstallDirectory, _failureFilesFolder);
        ...
        public static bool Fuck()
        {
            ...
            ApplicationLicenseManagerExtensionPoint xp = new ApplicationLicenseManagerExtensionPoint();
            ILicenseManager manager = (ILicenseManager)xp.CreateExtension();
            if (manager == null)
            {
                return false;
            }
            return manager.IsGivedLicense();
