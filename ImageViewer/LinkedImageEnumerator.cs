    ...
    internal class LinkedImageEnumerator : IEnumerable<IPresentationImage>
    {
        ...
        private IEnumerable<IPresentationImage> GetAllLinkedImages()
        {
            ...
            else
            {
                ...
                {
                    if (_referenceImage.ImageViewer != null && _referenceImage.ImageViewer is PrintImageViewerComponent)
                    {
                        var printComponent = _referenceImage.ImageViewer as PrintImageViewerComponent;
                        
                        if (printComponent.SelectPresentationImages.Contains(image as PresentationImage))
                        {
                            yield return image;
                        }
                        //var sopReference = (IImageSopProvider)_referenceImage;
                        //var sop = (IImageSopProvider)image;
                        //if (sopReference.Frame.ParentImageSop.StudyInstanceUid == sop.Frame.ParentImageSop.StudyInstanceUid &&
                        //    sopReference.Frame.ParentImageSop.SeriesInstanceUid == sop.Frame.ParentImageSop.SeriesInstanceUid
                        //    )
                        //{
                        //    yield return image;
                        //}
