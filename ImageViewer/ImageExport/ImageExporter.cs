...
namespace Macro.ImageViewer.ImageExport
{
	public abstract class ImageExporter : IImageExporter
	{
        ...
        //        return ImageDrawToBitmap(image, imageSize.Width, imageSize.Height, dpi);
		...
		public static Bitmap ImageDrawToBitmap(IPresentationImage presentationImage, int width, int height, float dpi)
		{
		    ...
		    var bitmapInfoString = String.Format("Bitmap��Height:{0},Width:{1}", width, height);
		    Platform.Log(LogLevel.Debug, bitmapInfoString);
			var graphics = System.Drawing.Graphics.FromImage(bmp);
		    IntPtr contextId=IntPtr.Zero;
		    try
		    {
		        contextId = graphics.GetHdc();
