...
{
	public sealed class PrintViewerShortcutManager : IViewerShortcutManager
	{
		private readonly Dictionary<ITool, ITool> _tools;
        private readonly PrintImageViewerComponent _viewer;

        public PrintViewerShortcutManager(PrintImageViewerComponent viewer)
		{
		...
		public IClickAction GetKeyboardAction(KeyboardButtonShortcut shortcut)
		{
            ...
            var action = FindClickAction(shortcut, _viewer.ActionsNamespace, PrintImageViewerComponent.PrintKeyboardSite, actions);

			// if not defined, give consideration to the viewer context menu
			if (action == null)
                action = FindClickAction(shortcut, _viewer.ActionsNamespace, PrintImageViewerComponent.PrintContextMenuSite, actions);
			...
			if (action == null)
			{
				foreach (var site in CollectionUtils.Unique(CollectionUtils.Map<IAction, string>(actions, a => a.Path.Site)))
				{
					if (site != globalMenusActionSite
                        ...
                        && site != PrintImageViewerComponent.PrintKeyboardSite
                        && site != PrintImageViewerComponent.PrintContextMenuSite)
					{
