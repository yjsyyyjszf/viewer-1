...
{
    [ExtensionPoint]
    public sealed class PrintPreviewImageBoxViewExtensionPoint : ExtensionPoint<IView>
    { }


    [AssociateView(typeof(PrintPreviewImageBoxViewExtensionPoint))]
    public class PrintViewImageBox : IPrintViewImageBox
    {
        #region 私有成员
        private IImageBox _parentImageBox;
        private IImageBox _upNodeImageBox;
        private IImageBox _nextNodeImageBox;
        private ImageBoxCollection _childImageBoxs;
        private List<PrintViewTile> _selectTiles;
        #endregion

        #region Private Fields

        private TileCollection _tiles;
        private PrintImageViewerComponent _imageViewer;
        private PhysicalWorkspace _parentPhysicalWorkspace;
        private DisplaySet _displaySet;
        private PrintViewTile _selectedTile;
        ...
        private int _topLeftImageIndex = 0;

        private int _totleImagesCount = 0;//所有现有Image的数量(包括空页的Tile数量)
        ...
        private List<PresentationImage> _selectPresentationImages = new List<PresentationImage>();
        ...
        #region IPrintViewImageBox 成员

        public IImageBox ParentImageBox
        {
            get { return _parentImageBox; }
            set { _parentImageBox = value; }
        }

        public IImageBox UpNodeImageBox
        {
            get { return _upNodeImageBox; }
            set { _upNodeImageBox = value; }
        }

        public IImageBox NextNodeImageBox
        {
            get { return _nextNodeImageBox; }
            set { _nextNodeImageBox = value; }
        }

        public ImageBoxCollection ChildImageBox
        {
            get
            {
                if (_childImageBoxs == null)
                {
                    _childImageBoxs = new ImageBoxCollection();
                }
                return _childImageBoxs;
            }

            internal set { _childImageBoxs = value; }
        }

        public List<PrintViewTile> SelectTiles
        {
            get
            {
                if (_selectTiles == null)
                {
                    _selectTiles = new List<PrintViewTile>();
                }
        ...
        public int TotleImageCount
        {
            get { return _totleImagesCount; }
            set
            {
                _totleImagesCount = value;
                if (_totleImagesCount < 0)
                {
                    _totleImagesCount = 0;
                }
            }
        ...
        public IImageViewer ImageViewer
        {
            ...
            internal set
            {
                _imageViewer = value as PrintImageViewerComponent;

                if (_imageViewer != null)
                {
                    foreach (PrintViewTile tile in this.Tiles)
        ...
        private void PresentationImageAdd(object sender, ListEventArgs<IPresentationImage> args)
        {
            if (DisplaySet.PresentationImages.Count > _totleImagesCount)
            {
                _totleImagesCount = _totleImagesCount + 1;
            }
        }

        private void PresentationImageRemove(object sender, ListEventArgs<IPresentationImage> args)
        {
            _totleImagesCount = _totleImagesCount - 1 > 0 ? _totleImagesCount - 1 : 0;
        ...
        public ITile SelectedTile
        {
            ...
            set
            {
                _selectedTile = value as PrintViewTile;
        ...
        public int TopLeftPresentationImageIndex
        {
            ...
            set
            {
                ...
                _totleImagesCount = CaclPageCountHelper.CaclPageCount(_totleImagesCount, Tiles.Count) * Tiles.Count;

                int imageIndex;

                if (value < 0)
                    imageIndex = 0;
                else if (value > (_totleImagesCount - Tiles.Count))
                    imageIndex = _totleImagesCount - Tiles.Count;
                else
                    imageIndex = value;

                _topLeftImageIndex = imageIndex;
        ...
        private void Select()
        {
            if (!this.Selected)
            {
                ...
                this.Selected = true;
                var printImageViewerComponent = this.ImageViewer as PrintImageViewerComponent;
                printImageViewerComponent.SelectedImageBox = this;
        ...
        {
            get { return _selectPresentationImages; }
        }
        ...
        {
            //Platform.CheckArgumentRange(imageIndex, 0, _totleImagesCount, "startIndex");

            for (int tileIndex = 0; tileIndex < this.Tiles.Count; tileIndex++)
                SetImage(imageIndex, tileIndex);
