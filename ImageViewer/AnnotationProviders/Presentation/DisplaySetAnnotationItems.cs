    ...
    internal class DisplaySetNumberAnnotationItem : AnnotationItem
    {
        ...
        public override string GetAnnotationText(IPresentationImage presentationImage)
        {
            if (presentationImage != null && presentationImage.ParentDisplaySet != null)
            {
                ...
                {
                    if (presentationImage.ImageViewer is ImageViewerComponent)
                    {
                    ...
                    {
                        var frame = (IImageSopProvider) presentationImage;
                        return string.Format(SR.FormatDisplaySetNumberAndCount, frame.Frame.ParentImageSop.SeriesNumber, frame.Frame.ParentImageSop.ParentSeries.ParentStudy.Series.Count);
