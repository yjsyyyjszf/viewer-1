...
namespace Macro.ImageViewer
{
    ...
    public partial class ImageViewerComponent : ApplicationComponent, IImageViewer, IContextMenuProvider
    {
        ...
        public ImageViewerComponent()
            : this(LayoutManagerCreationParameters.Simple)
        {
        }
        ...
        public string PatientsLoadedLabel
        {
            get
            {
                int seriesCount = 0;
                int sopCount = 0;
                foreach (ImageSet imageSet in this.LogicalWorkspace.ImageSets)
                {
                    foreach (DisplaySet displaySet in imageSet.DisplaySets)
                    {
                        seriesCount++;
                        sopCount = sopCount + displaySet.PresentationImages.Count;
                    }
                }
                string tile = CreateTitle(CollectionUtils.Map(this.StudyTree.Patients, (Patient patient) => (IPatientData)patient));
                string seriesCountString = string.Format(SR.FormatSeriesAndSopCountString, seriesCount, sopCount);
                return StringUtilities.Combine<string>(new string[] { tile, seriesCountString }, String.Format(" {0} ", SR.SeparatorPatientDescription));
            }
        }
